'''
1 make a proper level
https://www.google.com/search?q=robo+rally+levels&safe=strict&rlz=1C1CHXU_enUS715US715&source=lnms&tbm=isch&sa=X&ved=0ahUKEwjHuPrG8uHgAhUIKawKHa28DMYQ_AUIECgD&biw=1440&bih=757
You should probably have progressively more
complicated levels. First level should have no obstacles.
...
4 Detecting the win condition and ending the game.
...
Get it working for yourself first so you know how. Lead them to it through
little investigations. Stacks and queues and then robo rally can carry you
through the end of the year.
7 An AI that CS1 could write.
8 Improve AI1 to make it navigate around holes.
9 Guantlet with limited moves per level and a score at the end.
...
class AI1(Robot):
    ...
    move a robot. All non-conveyor tiles return 0,0,0.
        ...
        if self.oscillate:
            ...
            flag = flags[0].color
            flag_row = flags[0].row
            flag_col = flags[0].col
            for f in flags:
...
player1 = Waitbot(surface, white)
player2 = AI1(surface, yellow)
player3 = Robot(surface, red) #Random bot
player4 = Robot(surface, red) #Random bot
