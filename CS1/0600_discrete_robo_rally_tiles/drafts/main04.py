...
class Robot:
    ...
    def doAction(self, board, action):
        if action == 'left':
            self.rotateLeft()
        elif action == 'right':
            self.rotateRight()
...
while not done:
    ...
    #TODO
    action_list = []
    action_list.append([player1, player1.update(board)])
    action_list.append([player2, player2.update(board)])
    random.shuffle(action_list)
    for player, action in action_list:
        player.doAction(board, action)
        drawAll() #Draw and pause between each action
pygame.quit()