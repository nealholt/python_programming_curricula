...
def getAllNeighbors(board,row,col, ignore=[]):
    '''This function should return all neighbors in order of north, east,
    south, west (same order as direction_list) to the cell in row,col.
    None will be returned in place of a cell if there is a wall on that side.
    Cells in the ignore list (which contains row,col pairs) will not be
    included. Lastly, conveyors will be accounted for.
    For example: If the cell to the right conveys up
    then the actual right neighbor is the cell diagonally up and right.'''
    ...
    for i in range(len(direction_list)):
        ...
        if isConveyor(board, temp_row, temp_col):
            temp_row,temp_col,_ = conveysTo(board, temp_row, temp_col)
