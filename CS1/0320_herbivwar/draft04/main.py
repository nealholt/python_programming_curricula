import pygame, block, critter, critterAI1, critterAI2
...
critter_cells[row][col] = critterAI1.CritterAI1(screen, row, col, frames[22], team, team0_name)
critter_cells[row][col].direction_index = 3 #face west
...
round_limit = 150
count = 0
while not done and count<round_limit:
    count += 1
    #Handle events
        ...
        for col in range(tiles):
            if critter_cells[row][col]!=None:
                if critter_cells[row][col].team==0:
                    team0+=1
                if critter_cells[row][col].team==1:
                    team1+=1
    #Display winner text
    text = ''
    if team0>team1:
