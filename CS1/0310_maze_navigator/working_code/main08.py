...
maze_file = '../maze_navigator_starter_code/mazes/trial05.txt'
...
'''TODO
	-Put this step off indefinitely: Write combination wall following and open maze navigation version that tracks distance to goal. See notes in the comment for randomHybridNavigation
'''
...
class MazeRunner:
	...
	def randomHybridNavigation(self, maze, goal_row, goal_col):
		...
		The best deterministic algorithm that I know of uses open 
		navigation until it gets stuck at which point it remembers how far it
		was from the goal and then engages wall following until it gets 
		closer than when it got stuck, at which point it reverts to wall
		following.
	...
	def handWallNavigation(self, maze):
		...
		This strategy is stumped by trial04, when the goal is not against a wall, but does well up until then.'''
		self.turnLeft()
