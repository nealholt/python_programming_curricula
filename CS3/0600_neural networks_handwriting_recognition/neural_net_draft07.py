...
def backpropagate(nn, eta, target_outputs):
    ...
    for i in range(len(nn.layers[-1])):
        ...
        derror_douti = const*(target-out_neuron.out)
        #Backpropogate
        out_neuron.backpropagate(derror_douti, eta)
...
def sigmoid(z):
    ...
    #This old way was slower and speed became an issue.
...
class Neuron:
    def __init__(self, input_neurons):
        ...
        #Remember number of inputs as a time saver when making prediction and
    ...
    def backpropagate(self, derror_dout, eta):
        #Base case: we are at an input node (which has no weight) or the
        #amount of error was zero (in which case there is no learning to do)
        if self.num_inputs==0 or derror_dout==0:
            return
        #Calculate derivative of output with respect to net_input
        #dout_dinput = derror_dout * -(sigmoid(self.net_in)**2)*(2.718281828459045**(-self.net_in))
        dout_dinput = derror_dout * -(self.out**2)*(2.718281828459045**(-self.net_in))
        #The above is slightly faster than the former:
        #dout_dinput = derror_dout * -(sigmoid(self.net_in)**2)*(math.exp(-self.net_in))
        #The latter part of the calculation: -(sigmoid(self.net_in)**2)*(2.718281828459045**(-self.net_in))
        #is ready to calculate as soon as forward propagate is done. There
        #might be further speed gains to be made by moving this calculation
        #to forwardProp and storing it in a variable in the neuron, because
        #backprop is called a few times over

        #Loop over inputs to learn and to backpropagate recursively
        for i in range(self.num_inputs):
            #The error will be zero if the input was zero so we can save
            #time and stop here
            if self.inputs[i].out != 0:
                #update the weight of the output neuron
                derror_dweight = dout_dinput * self.inputs[i].out
                #Learn
                self.weights[i] -= derror_dweight * eta
                #Backprop recursively
                self.inputs[i].backpropagate(derror_dweight, eta)
