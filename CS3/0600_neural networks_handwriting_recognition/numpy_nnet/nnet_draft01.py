...
data=np.loadtxt('../data/train.csv',skiprows=1,delimiter=",",
                usecols = range(1,784), max_rows=row_count)
...
def getData():
    ...
    row_count = int(line_count*training_percent)
    col_count = 784
    X = np.empty((row_count,col_count))
    Y = np.empty((1,col_count))
...
def parseLine(file_handle):
    '''Reads in the next line and parses it into a 2d array of
    gray scale values.'''
