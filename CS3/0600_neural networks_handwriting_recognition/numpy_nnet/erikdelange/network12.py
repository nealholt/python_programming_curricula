...
'''This function takes a network (presumably that which
we're working on) and saves its weights in a text file
using the numpy.savetxt function. Each of the weights is
deliniated by a comma, and will be formatted as a .gz
file, which the numpy.loadtext function can easily read.

The file name will depend on a second variable, which,
if not given, will ask the user to input the file name.
This is in place to prevent this file from saving over
an already existing weights file.'''
def saveWeights(network, file_name="New_Weights.gz"):
    w = network.weight
    '''
    print("Length of Outermost Array:",len(w))
    print("Length of first entry in outermost array:",len(w[0]))
    print("Length of second entry in outermost array:",len(w[1]))
    print()
    print("Lengths of entries in first entry in outermost array.")
    for i in range(len(w[0])):
        print(str(i + 1) + " Entry: " + str(len(w[0][i])))
    print()
    print("Lengths of entries in second entry in outermost array.")
    for i in range(len(w[1])):
        print(str(i + 1) + " Entry: " + str(len(w[1][i])))
    '''
    np.asarray(w).tofile(file_name, sep=',')

def loadWeights(file_name="New_Weights.gz"):
...
input_count = 785
row_count = 4000 #Train on this many rows at a time
...
network = NN((784, 50, 50, 10), (None, sigmoid, sigmoid, sigmoid))

error = network.train(data, targets, iterations=5000, learning_rate=2)
...
#saveWeights(network, file_name = "New_Weights.gz")
#loadWeights(file_name = "New_Weights.gz")
