...
row_count = 40000 #Train on this many rows at a time
...
for num in targets:
    ...
    #Sanity check
    #PASSED SANITY CHECK; REFORMATTING SUCCESSFUL
    if sum(temp) != 1:
        print('ERROR: Target reformatting fail.')
        print('Reformatted:',temp)
        print('Original:',num)
        exit()
e_targets = np.asarray(e_targets)

network = NN((784, 50, 10), (None, sigmoid, sigmoid))

error = network.train(data, e_targets, iterations=50, learning_rate=0.1)
...
#Next line added for handwriting recognition
#e_y_hat = processOutput(y_hat)
loss_values = (e_targets - Y_hat).T

print("predict:", Y_hat.T)
print("desired:", e_targets.T)
print("loss   :", loss_values)

#TODO Convert Y_hat and e_targets to integers to check if they're equal, and
#get a sense of the program's accuracy.
