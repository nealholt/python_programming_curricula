...
class PhysicsObject:
    ...
    def collidedCircular(self, other):
    ...
    def turnToward(self, x,y,close_enough=0):
        goal_angle = self.angleTo(x,y)
        turn = self.getSmallestTurnAngle(goal_angle)
        if turn > close_enough:
            self.rotateClockwise()
        elif turn < -close_enough:
            self.rotateCounterClockwise()

    def turnFrom(self, x,y,close_enough=0):
        goal_angle = self.angleTo(x,y)
        turn = self.getSmallestTurnAngle(goal_angle)
        if turn > close_enough:
            self.rotateCounterClockwise()
