...
while not done:
    ...
    '''data = player.getSensorData()
    print('data '+str(data))
    nn.predict(data) #Send data to neural net
    outputs = nn.getOutputs() #Get response from neural net
    print('nn outputs '+str(outputs))
    player.respondToInput(outputs) #Control car with the response
    player.moveForward()'''
    print()
