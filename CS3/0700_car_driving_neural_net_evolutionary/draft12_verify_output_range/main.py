...
for _ in range(population_size):
    ...
    cars.append(c)
    b = brain.NeuralNet([7,20,2])
    brains.append(b)

#Set this value higher to draw less frequently for a minor speed gain
#Set this to 1 for drawing every frame.
stutter = 30
...
while not done:
    ...
    for i in range(len(cars)):
        if not cars[i].crashed:
            ...
            if outputs[0] < min_outputs[0]:
                min_outputs[0] = outputs[0]
            if outputs[1] < min_outputs[1]:
                min_outputs[1] = outputs[1]
            if outputs[0] > max_outputs[0]:
                max_outputs[0] = outputs[0]
            if outputs[1] > max_outputs[1]:
                max_outputs[1] = outputs[1]
