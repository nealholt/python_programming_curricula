...
0. Enemy selects different targets. Basic AI
1. multiple fighting enemies
2. consolidate draw code. Enable camera switching.
3. Experience
4. Leaderboard
5. Organize attributes that can be leveled up.
5A. Level up attributes.
6. Differentiated exp bricks.
7. Regenerating health. (Or a shield?)
8. upgrades, how to check for and choose upgrades
9.

Later: strafe, get intercept vector, separate direction shooting
from direction moving.
Consider energy costs to do radar pings, sweeps, or spot checks.
Return list of object copies so info becomes outdated. Energy
cost to move and shoot.
...
while not done:
    ...
    for a in all_sprites:
        if a.sprite_type == TYPE_CRITTER:
            visible_sprites = getSpritesInRange(all_sprites, a.vision_radius, a.x,a.y)
