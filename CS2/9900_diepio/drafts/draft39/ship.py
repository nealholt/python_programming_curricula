import diep_sprite, math, funcs, random, bullet, bullet_seeking
...
class Ship(diep_sprite.DiepSprite):
    def __init__(self, screen, x, y, color, radius, name):
        ...
        #Whether or not to use seeking bullets
        self.use_seeking = False

    def becomeSeekerShooter(self):
        '''Shoots seeking bullets'''
        self.revertAll() #revert to baseline values
        self.use_seeking = True
        #Low recoil
    ...
    def shootOneBullet(self, angle):
        ...
        if self.use_seeking:
            b = bullet_seeking.BulletSeeking(self.screen, self.x, 
                        self.y, red,
    ...
    def moveTowardLocation(self,x,y):
        if not self.can_move:
            ...
            return
        self.can_move = False
        self.moveTowardLocationAccel(x,y,self.xs[ACCEL_INDEX])
