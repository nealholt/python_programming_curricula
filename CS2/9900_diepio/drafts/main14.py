...
'''TODO next steps:
1. Implement shooting
2. Implement health bars
3. Drawing can be more efficient. Only draw things that are on
the screen, for one thing.
4. increase collision efficiency by sorting collision lists.
...
class PlayerCritter(DiepSprite):
    def __init__(self, screen, x, y, color, radius, accel):
        super().__init__(screen, x, y, color, radius, accel, sprite_type=TYPE_CRITTER)
...
while not done:
    ...
    for i in range(len(all_sprites)-1):
        for j in range(i+1,len(all_sprites)):
            if all_sprites[i].depthOfPenetration(all_sprites[j]) > 0:
