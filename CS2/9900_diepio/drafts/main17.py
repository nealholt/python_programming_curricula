...
0.
1.
2. increase collision detection efficiency by sorting collision lists.
3. Reset dead food (reset and relocate as if it was destroyed and regenerated)
4. Prevent food on food damage.

Cal and/or Brendan help develop parts of this?

Recoil
differentiated exp bricks.
Level up attributes.
3 lives.
    ...
    for i in range(len(all_sprites)-1):
        for j in range(i+1,len(all_sprites)):
            ...
            if all_sprites[i].collidedWith(all_sprites[j]):
                ...
                all_sprites[i].takeDamage(1)
                all_sprites[j].takeDamage(1)

    #Delete dead sprites
    for i in reversed(range(len(all_sprites))):
        if all_sprites[i].sprite_type == TYPE_BULLET and all_sprites[i].timeout<0:
            del all_sprites[i]
        elif all_sprites[i].hp <= 0:
            del all_sprites[i]
