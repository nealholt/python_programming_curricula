...
This is currently a fresh draft.

DONE 3. display attribute levels on the screen.
DONE Created levelup_text for this and displayLevels function in main.
DONE 3A Write in a number to press to level up the given attribute
DONE 3B Display experience bar
DONE 3C implement the number key press to gain the level up
DONE 3D display amount of level ups ready to spend

DONE 3E in diep_sprite levelUp. Need to fully understand
the xs list and how to update it in order to complete this. We want
to reset total hp and such based on the new level, but not current hp.

self.xs is the current value of a bunch of different attributes.
The getDefault function is then used to figure out the baseline values.

Basically health needs separated out of the self.xs list because health
isn't static like all the other attributes. The MAX health can
still be kept in the xs list.

4. Move more functions out of main and into functions?
DONE 5. make level ups cost experience.
6. some powerups are still making this freeze. Create a
test scenario for this with all powerups lines up.
7. do the powerups all work?
8. should put the enemies back in for testing
9. consider what is minimal to make a competitive game.
...
import time, pygame, diep_sprite, player, enemy, funcs, sparkle, powerup, random, food
