...
class DiepSprite:
    def __init__(self, screen, x, y, color, radius, accel, hp,
        ...
        self.bullet_longevity = default_bullet_timeout #number of frames until a bullet dies
        self.bullet_hp = bullet_hp #Sometimes known as bullet penetration
        self.invincibility_countdown = 0
        self.spread_shot_countdown = 0 #TODO
        self.big_shot_countdown = 0
        self.speed_boost_countdown = 0
        self.shove_countdown = 0
    ...
    def shoot(self):
        if self.refire_count <= 0:
            ...
            b = Bullet(self.screen, self.x, self.y, red, bullet_size,
                        dx * self.bullet_speed, dy * self.bullet_speed,
                        self.bullet_hp, self.bullet_damage,
                        self.bullet_longevity, self)
            global all_sprites
    ...
    def takeDamage(self, damage):
        ...
        in negative damage.'''
        #No damage on invincible critters, but they can still heal
        if self.invincibility_countdown > 0 and damage > 0:
            return
