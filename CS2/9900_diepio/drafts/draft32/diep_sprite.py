...
class DiepSprite:
    ...
    def levelUp(self, level_index):
        ...
        if potential_level-current_level > 0:
            self.levels[level_index] += 1
            #Adjust all attributes based on new level
            #TODO LEFT OFF HERE
    ...
    def resetHealthbar(self,screen,hp):
        if self.draw_healthbar:
            self.xs[HP_INDEX] = hp
            self.default_food_hp = hp
            self.healthbar = health_bar.HealthBar(screen,0,0,self.radius*2,10,hp)
    ...
    def getDefault(self,index):
        ...
        if self.sprite_type==TYPE_FOOD and index==HP_INDEX:
            return self.default_food_hp
        else:
