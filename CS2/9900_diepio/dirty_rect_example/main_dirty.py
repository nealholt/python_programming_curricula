...
'''
This is the dirty rect version of main_clean, hopefully to
see a speed up.
'''
...
def getNewEnemy():
    return sprite.Sprite(screen,
            random.randint(0, screen.get_width()-20), 0, 30,
            random.randint(-5,5), 3, 'ellipse', green,
            4, 'enemy')
...
while not done:
    start = time.time()
    #Empty/reset the dirty rects list
    dirty_rects = []

    #Blit a piece of the background over the sprite's current
    #location, erasing it.
    for sp in all_sprites:
        pygame.draw.rect(screen, black, sp.rect)
        #Append the sprite's current location rectangle to a list
        #called dirty_rects.
        dirty_rects.append(sp.rect.copy())
