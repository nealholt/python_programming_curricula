...
#371 lines of code before, but with inheritance it's down to 345 lines of code!
...
def screenWrap(x, y):
    '''Given a pair of coordinates, returns adjusted coordinates
    so that objects on screen will wrap if they move off screen.'''
    global width, height
    if x < 0:
        x += width
    elif x > width:
...
class Ballistic:
    def __init__(self, screen, x, y, dx, dy, radius):
        self.screen = screen
        self.x = x
        self.y = y
        self.dx = dx
    ...
    def move(self):
        ...
        self.x += self.dx
        self.y += self.dy
        self.x, self.y = screenWrap(self.x+self.radius, self.y+self.radius)
        #Subtract back out the radius
        self.x -= self.radius
        self.y -= self.radius


class Line(Ballistic):
    def __init__(self, screen, x1, y1, x2, y2, dx, dy):
        ...
        Ballistic.__init__(self, screen, (x2+x1)/2, (y2+y1)/2, dx, dy, 0)
...
class Bullet(Ballistic):
    def __init__(self, screen, x, y, angle):
        speed = 10
        Ballistic.__init__(self, screen, x, y, math.cos(angle)*speed, math.sin(angle)*speed, 2)
    ...
    def draw(self):
        ...
        pygame.draw.ellipse(self.screen, white, r)
...
class Ship(Ballistic):
    def __init__(self, screen, x, y):
        Ballistic.__init__(self, screen, x, y, 0, 0, 20)
        self.speed = 0.3
        self.sides = 3
        self.angle = 0
...
class Asteroid:
    def __init__(self, screen, x, y, dx, dy, min_radius, max_radius):
        self.min_radius = min_radius
        self.max_radius = max_radius
        avg_radius = (self.min_radius+self.max_radius)/2
        Ballistic.__init__(self, screen, x, y, dx, dy, avg_radius)
    ...
    def draw(self):
        ...
        pygame.draw.polygon(self.screen, white, point_list, self.line_thickness)
