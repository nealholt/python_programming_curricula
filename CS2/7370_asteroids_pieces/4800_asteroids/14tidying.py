...
def shoot(screen, player, bullet_list, list_manager):
	#Shoot
	x = player.x+math.cos(player.angle)*player.radius
	y = player.y+math.sin(player.angle)*player.radius
	b = Bullet(screen, x, y, player.angle)
	bullet_list.append(b)
	list_manager.addToList(b)
...
#Create list manager object to manage updating and moving everything
lm = ListManager(asteroid_list + bullet_list + [player])
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			...
			if event.key == pygame.K_SPACE:
				shoot(screen, player, bullet_list, lm)
	if player.lives > 0:
