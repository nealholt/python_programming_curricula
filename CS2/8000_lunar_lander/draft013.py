...
'''
Draw clouds
    screen wrap clouds (or only keep them relatively above the player?)
    remove clouds when not needed or only draw them when on screen
Draw stars
...
class GameManager:
    def __init__(self, screen, level):
        ...
        for i in range(10):
            ...
            y = int(atmo_upper_limit/2+random.randint(-3*atmo_layer_width,3*atmo_layer_width))
...
class Clouds:
    def __init__(self, surface, x, y):
        ...
        for i in range(num_puffs):
            self.xs.append(x+random.randint(-20,20))
            self.ys.append(y+random.randint(-20,20))
        self.remove_me = False
