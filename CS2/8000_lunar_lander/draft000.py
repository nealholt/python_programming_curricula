...
class Bullet:
    ...
    def draw(self, povx, povy):
        ...
        r = pygame.Rect(x_adjust + self.x-self.radius,
                        y_adjust + self.y-self.radius,
...
class Ground:
    def __init__(self, surface):
        ...
        self.ground_heights = []
        for i in range(10):
            self.ground_heights.append([i*200,
                                self.surface.get_height()/2])#+random.randint(100,300)]) #TODO
    ...
    def draw(self, povx, povy):
        ...
        for coord in self.ground_heights:
            ...
            if coord[0]+povx > 0 and coord[0]+povx < self.surface.get_width():
                adjusted_heights.append([coord[0]+povx, coord[1]+povy])
        #print(len(adjusted_heights))
        #print(adjusted_heights[0])
        #TODO
        #pygame.draw.polygon(self.surface, white, adjusted_heights, 3)
        coord1 = [0, y_adjust+self.surface.get_height()/2]
        coord2 = [self.surface.get_width(), y_adjust+self.surface.get_height()/2]
        pygame.draw.line(self.surface, white, coord1, coord2, 3)
