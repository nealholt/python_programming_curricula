...
class GameManager:
    def __init__(self, screen, level):
        ...
        for i in range(cloud_count):
            x = random.randint(0,ground_total_width)
...
class Ground:
    def __init__(self, screen):
        ...
        self.ground_heights = [[0,random.randint(ground_max_height,ground_min_height)]]
        #While the most recently added x value is less than the total
        #desired ground width, keep adding new random points.
        while self.ground_heights[-1][0] < ground_total_width:
            new_point = [self.ground_heights[-1][0]+random.randint(ground_min_width, ground_max_width),
                        random.randint(ground_max_height,ground_min_height)]
            self.ground_heights.append(new_point)
    ...
    def getHeightAt(self, x):
        ...
        if x<=self.ground_heights[0][0] or x>=self.ground_heights[-1][0]:#TODO change this since ground now wraps
    ...
    def draw(self, x_adjust, y_adjust):
        ...
        #Find the coordinate to start the ground at. The ground will wrap
        #as the player moves across the world. We are looking for the coordinate
        #that is just barely off the left edge of the screen.
        ground_start_index = 0
        extra_adjust = 0
        if self.ground_heights[0][0]+x_adjust > 0:
            extra_adjust = -ground_total_width
        for i in range(len(self.ground_heights)-1):
            #If one coordinate is off screen and the next is on, then we've
            #found our starting point.
            if self.ground_heights[i][0] + extra_adjust + x_adjust <= 0 and \
            self.ground_heights[i+1][0] + extra_adjust + x_adjust > 0:
                ground_start_index = i
                break
        #Find the coordinate to end the ground at. The ground will wrap
        #as the player moves across the world so this coordinate might
        #come BEFORE the start. That's ok.
        ground_end_index = 0
        extra_adjust = 0
        if self.ground_heights[-1][0]+x_adjust < width: #width is screen width
            extra_adjust = ground_total_width
        for i in range(len(self.ground_heights)-1):
            #If one coordinate is on screen and the next is off, then we've
            #found our starting point.
            if self.ground_heights[i][0] + extra_adjust + x_adjust <= width and\
            self.ground_heights[i+1][0] + extra_adjust + x_adjust > width:
                ground_end_index = i+1
                break
        #Adjust all the other coordinates
        #Figure out any adjustment for the screen wrap
        extra_adjust = 0
        if ground_start_index > ground_end_index and x_adjust < ground_total_width/2+self.screen.get_width()/2:
            extra_adjust = -ground_total_width
        #Add in adjusted coordinates
        i = ground_start_index
        while i!=ground_end_index:
            coord = self.ground_heights[i]
            temp = [coord[0]+x_adjust+extra_adjust, coord[1]+y_adjust]
            ...
            if onScreen(temp[0], temp[1]):
                is_visible = True
            #detect screen wrap and adjust x coordinate accordingly
            old_i = i
            i = (i+1)%len(self.ground_heights)
            if i < old_i:
                extra_adjust += ground_total_width
