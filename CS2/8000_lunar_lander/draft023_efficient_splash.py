...
'''
Splash could be a lot better optimized:
    not drawing splashes that are obscured by other splashes
    not drawing off-screen splashes
...
class SplashManager:
    ...
    def update(self, player):
        ...
        if player.y > water_upper_limit-splash_limit and player.y < water_upper_limit+wave_radius:
            ...
            #Create splash centered beneath player
            #self.splash_list.append(Splash(self.screen, x, wave_radius+1, -amount_splash))
            self.addSplash(Splash(self.screen, x, wave_radius+1, -amount_splash))
            ...
            for i in range(1,splash_spread):
                ...
                #self.splash_list.append(Splash(self.screen, x+i*splash_width, wave_radius+1, splash_force))
                #self.splash_list.append(Splash(self.screen, x-i*splash_width, wave_radius+1, splash_force))
                self.addSplash(Splash(self.screen, x+i*splash_width, wave_radius+1, splash_force))
                self.addSplash(Splash(self.screen, x-i*splash_width, wave_radius+1, splash_force))

    def addSplash(self, splash):
        ...
        for s in self.splash_list:
            if s.x == splash.x:
                s.dy = min(s.dy, splash.dy)
                s.height = max(s.height, splash.height)
                added = True
                break
        if not added:
