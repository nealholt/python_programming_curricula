...
'''
1 put this stuff in folders and separate the objects into different files
2 put a landing pad back in
3 print a score for how good the landing is in terms of angle dx and dy
4 make the ground wrap around
5 make the graphics less processor consuming
6 Screen wrap stars and clouds differently to make it not jerk.

Complete game:
Levels to complete for high score.
Levels are beaten by acquiring certain amount of money.
Score is based on time only.
Levels are little solar systems to trade in.
Features: Use gravity sling shot to conserve fuel, use
atmopheric braking to conserve fuel, scoop fuel out of
upper atmosphere in dangerous maneuver, mine resources
in asteroid belt, trade goods between locations in dynamic
economy, upgrade ship to be able to visit hot planets,
high gravity planets, high radiation planets, etc. Upgrade
cargo space, aerodynamic-ness, thrust. Buy weapons to fight
off pirates or just flee from them.

Contrails - thin straight lines off of edgepoints of ship in opposite
direction of line of motion, appearing only in atmosphere.
...
ground_total_width = width*6 #6 screen widths
...
class Clouds(Animation):
    ...
    def draw(self, x_adjust, y_adjust):
        ...
        if y>-cloud_size and y-cloud_size<height and \
        x>-cloud_size and x-cloud_size<width:
            #Draw the cloud
            for i in range(len(self.xs)):
                pygame.draw.ellipse(self.screen, white, [self.xs[i]+x_adjust, self.ys[i]+y_adjust, cloud_size, cloud_size])
        '''#TODO #Check for screen wrapping
        if x < -width/2:
            self.x += 2*width
        if x > width+width/2:
            self.x -= 2*width'''
