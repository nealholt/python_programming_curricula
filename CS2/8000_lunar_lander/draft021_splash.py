...
Pass in the adjustment to x and y pre calculated. Don't recalculate it every time.
...
class GameManager:
    ...
    def update(self):
        ...
        self.splash_manager.draw(self.player.x, self.player.y)
...
class SplashManager:
    ...
    def update(self, player):
        ...
        if player.y > water_upper_limit-splash_limit and player.y < water_upper_limit+wave_radius:
            ...
            self.splash_list.append(Splash(self.screen, player.x, wave_radius, -amount_splash))
            #Create reduced splashes on either side of center
            for i in range(2,4):
                self.splash_list.append(Splash(self.screen, player.x+i*splash_width, wave_radius, -amount_splash/i))
                self.splash_list.append(Splash(self.screen, player.x-i*splash_width, wave_radius, -amount_splash/i))
