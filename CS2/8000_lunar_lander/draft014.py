...
'''
Standardize use of screen or surface
Add inheritance (especially to animation objects)
Draw stars
...
Change physics based on conditions: friction, lift when flying close to horizontal.
	...
	-friction is a function of height in atmospher, zero in space, sharp transition to high friction in water.
	-constant gravity
...
class Clouds:
    ...
    def move(self):
        pass #Later you could add wind and make them drift
    ...
    def draw(self, povx, povy):
        ...
        if y>-cloud_size and y-cloud_size<self.surface.get_height() and \
        x>-cloud_size and x-cloud_size<self.surface.get_width():
            #Draw the cloud
            for i in range(len(self.xs)):
                pygame.draw.ellipse(self.surface, white, [self.xs[i]+x_adjust, self.ys[i]+y_adjust, cloud_size, cloud_size])
