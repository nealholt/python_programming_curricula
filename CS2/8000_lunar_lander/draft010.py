...
class Ground:
    ...
    def draw(self, povx, povy):
        ...
        for i in range(-1000,0,100):
            ...
            atmo_bottom = max(i+y_adjust+100,0)
            if atmo_bottom!=atmo_top:
                if atmo_top<self.surface.get_height(): #If atmo is on screen
                    temp = 255+int(i*240/1000)
                    print(temp)
                    color = temp,temp,temp
                    pygame.draw.rect(self.surface, color, [0,atmo_top,self.surface.get_width(), min(100,self.surface.get_height()-atmo_top)])
                else:
                    return
        #Draw the water
        water_top = max(0+y_adjust, 0)
