...
'''
TODO:
---longer branches, shrink faster.

make the brown deeper.
change color to white before changing to green.
Or is the whiteness related to the x coordinate?
...
screen.setup (width=800, height=800, startx=500, starty=0)
...
def getChildColor(parent):
    ...
    new_red = int(max(min(parent_red+i*d_red+random.randint(-rand_color_adjust,rand_color_adjust), 255), 0))
    new_green = int(max(min(parent_green+i*d_green+random.randint(-rand_color_adjust,rand_color_adjust), 255), 0))
    new_blue = int(max(min(parent_blue+i*d_blue+random.randint(-rand_color_adjust,rand_color_adjust), 255), 0))
...
def getChild(parent, depth, left):
    ...
    rand_int = random.randint(0,angle_range+angle_range_increase*depth)
    if left:
