...
while running:
    ...
    for p in platform_list:
        r = hippo.getRect()
        while p.rect.colliderect(r):
            #If the top_center hit, set the hippo's dy to zero
            if p.rect.collidepoint(r.midtop):
                hippo.dy = 0
                hippo.can_jump = False
            #If the bottom center hit, then set can_jump
            #back to true
            elif p.rect.collidepoint(r.midbottom):
                hippo.dy = 0
                hippo.can_jump = True
                p.putAbove(hippo)
                break
            #rewind the hippo out of collision
            temp_index = memory_index-1
            if temp_index<0:
                temp_index = len(memory)-1
            hippo.x, hippo.y = memory[temp_index]
            r = hippo.getRect()
